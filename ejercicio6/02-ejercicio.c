...
void modificarRotacionDeImagen(imagen* original,imagen* modificada ) {
    strcpy(modificada->tipo,original->tipo);

    modificada->altura = original->anchura;
    modificada->anchura = original->altura ;
    ...
    for( int i = 0; i < modificada->altura ;i++ ) {
        for( int j = 0; j < modificada->anchura ;j++ ) {
            modificada->pixeles[i][j] = original->pixeles[col-j][i];
        }
    }
}
...
int main() {
    ...
    if( validarImagen(&original) ){
        ...
        modificarRotacionDeImagen(&original,&modificada);
